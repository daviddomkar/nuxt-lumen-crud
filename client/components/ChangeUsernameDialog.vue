...
export default defineComponent({
  ...
  setup(props: { editedUsername: string }, { root, emit }) {
    const { token, profile, setProfile } = authStore.useConsumer();

    const username = ref(props.editedUsername);
      ...
      if (await validator.validate()) {
        if (token.value) {
          try {
            ...
            await changeUsername(token.value, username.value);

            // @ts-ignore
            setProfile({ ...profile.value, username: username.value });
